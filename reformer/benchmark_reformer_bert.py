...
def create_setup_and_compute(
    ...
    bert: bool = False,
    no_reformer: bool = False,
    num_hashes: int = 2,
):

    csv_filename = csv_filename + '_hashes_{}'.format(num_hashes)
    csv_memory_filename = csv_memory_filename + '_hashes_{}'.format(num_hashes)

    # SPECIAL REFORMER BENCHMARKING TEST #
    model_names = ["ReformerLayer"]

    if no_reformer:
        model_names = []

    if bert:
        model_names.append("BertLayer")
    ...
    if save_to_csv:
        with open(csv_filename, mode="w") as csv_file, open(
            csv_memory_filename, mode="w"
        ) as csv_memory_file:

            assert len(model_names) > 0, "At least 1 model should be defined, but got {}".format(model_names)
            first_result_dict = results[model_names[0]]['results']

            fieldnames = ["model"] + [f"{bs}x{ss}" for bs in first_result_dict for ss in first_result_dict[bs]]
            ...
            for model_name in model_names:
                ...
                model_memory_results = {
                    f"{bs}x{ss}": memory_dict[bs][ss]
                    for bs in memory_dict
                    for ss in memory_dict[bs]
                }
            ...
            for slice_size in slice_sizes:
                if max_input_size is not None and slice_size > max_input_size:
                    ...
                    sequence = tf.stack(
                        [
                            tf.squeeze(
                                tf.constant(tokenized_sequence[:slice_size])[None, :]
                            )
                        ]
                        * batch_size
                    )
                    ...
                    try:
                        print(
                            "Going through model with sequence of shape", sequence.shape
                        )
...
def main():
    ...
    parser.add_argument(
        ...
        required=False,
        action="store_true",
        help="Don't perform speed measurments",
    )
    parser.add_argument(
        "--no_memory",
        required=False,
        action="store_true",
        help="Don't perform memory measurments",
    )
    parser.add_argument(
        "--torch",
        required=False,
        action="store_true",
        help="Benchmark the Pytorch version of the " "models",
    )
    parser.add_argument(
        "--torch_cuda",
        required=False,
        action="store_true",
        help="Pytorch only: run on available " "cuda devices",
    )
    parser.add_argument(
        "--torchscript",
        required=False,
    ...
    parser.add_argument(
        "--save_to_csv", required=False, action="store_true", help="Save to a CSV file."
    )
    parser.add_argument(
        "--csv_filename",
        required=False,
        default=f"results/results_{round(time())}.csv",
        help="CSV filename used if saving results to csv.",
    )
    parser.add_argument(
        "--csv_memory_filename",
        required=False,
        default=f"memory/memory_{round(time())}.csv",
        help="CSV filename used if saving results to csv.",
    )
    parser.add_argument(
        "--average_over",
    ...
    parser.add_argument(
        "--slice_sizes", nargs="+", type=int, default=[8, 64, 128, 256, 512, 1024]
    )
    parser.add_argument(
        "--bert",
        required=False,
        action="store_true",
        help="Also run analysis on Bert?"
    )
    parser.add_argument(
        "--no_reformer",
        required=False,
        action="store_true",
        help="Don't run on Reformer?"
    )
    parser.add_argument(
        "--num_hashes",
        type=int,
        required=False,
        default=2,
        help="Num hashes used in Reformer"
    )
